"""Training loop for model_v3 (MLA + MoE + Hyper-Connections + MTP).
...
With overrides:
    uv run python train_v3.py --micro_batch_size=8 --grad_accum_steps=32
Resume:
    uv run python train_v3.py --resume checkpoints_v3/model_005000.pt
"""
...
def _build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train GPT v3 (MLA+MoE+HC+MTP)")
...
def main() -> None:
    ...
    for step in range(start_step, cfg.max_steps):
        ...
        if step % cfg.hella_interval == 0 and step > 0:
            from eval_hellaswag_v3 import evaluate_hellaswag
